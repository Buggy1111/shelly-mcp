...
async def test_switch_set_off_calls_and_returns_state(wire: Any) -> None:
    out = await shelly_switch_set(device="dev", on=False, channel=0)
...
async def test_switch_toggle(wire: Any) -> None:
    out = await shelly_switch_toggle(device="dev", channel=0)
...
async def test_switch_set_toggle_after(wire: Any) -> None:
    await shelly_switch_set(device="dev", on=True, toggle_after_s=30)
...
async def test_light_set_picks_rgbw_method_from_component(wire: Any) -> None:
    ...
    await shelly_light_set(device="dev", rgb=[1, 2, 3], brightness=40)
...
async def test_light_set_validates_rgb_and_brightness(wire: Any) -> None:
    assert "error" in await shelly_light_set(device="dev", rgb=[1, 2])
    assert "error" in await shelly_light_set(device="dev", brightness=150)


async def test_cover_move_open(wire: Any) -> None:
    await shelly_cover_move(device="dev", action="open")
    assert ("Cover.Open", {"id": 0}) in wire.calls


async def test_cover_move_to_position(wire: Any) -> None:
    await shelly_cover_move(device="dev", action="", position=70)
    assert ("Cover.GoToPosition", {"id": 0, "pos": 70}) in wire.calls


async def test_cover_move_invalid_action(wire: Any) -> None:
    assert "error" in await shelly_cover_move(device="dev", action="sideways")
    assert "error" in await shelly_cover_move(device="dev", action="open", position=200)


async def test_mutations_are_audit_logged(wire: Any, tmp_path: Path) -> None:
    await shelly_switch_set(device="dev", on=False)
...
async def test_unreachable_device_returns_error_dict_not_exception(wire: Any) -> None:
    ...
    out = await shelly_switch_set(device="dev", on=True)
