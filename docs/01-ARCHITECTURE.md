...
**Why:** Simpler, fewer moving parts, and **fully unit-testable offline** (fake aiohttp session) — which matters because the LAN path can't be live-verified until WSL mirrored networking is up. The `/rpc` and Gen1 REST endpoints are stable documented APIs.
**Consequences:** No push/subscribe in v1.0 (we poll on demand — fine for an MCP). aioshelly stays a pinned dep for a possible v1.1 event bus. The digest auth + request shaping are unit-tested; the actual socket round-trip against real devices is the one piece still pending live verification (WSL networking + a device).
