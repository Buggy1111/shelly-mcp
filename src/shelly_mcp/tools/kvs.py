...
Cloud can't reach it (the backend raises UnsupportedOnCloud → ``{"error": …}`` via the
``backend_errors`` decorator). Reads are safe; set is audited; delete is confirm-gated.
"""
