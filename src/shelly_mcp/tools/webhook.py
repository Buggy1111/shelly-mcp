...
can't manage them (UnsupportedOnCloud → ``{"error": …}`` via ``backend_errors``). List is
safe; create/update are audited; delete is confirm-gated. ``cid`` is the component id the
event belongs to (e.g. switch ``0``).
"""
